 ...
 */
class SearchFilmsViewModel(private val repository: FilmsRepository) : ViewModel() {
    ...
    private val result: LiveData<SearchResult<Film>> = Transformations.map(queryLiveData) {
        repository.searchFilm(it)
    }

    val films: LiveData<List<Film>> = Transformations.switchMap(result) { it.data }
    ...
    fun listScrolled(visibleItemCount: Int, lastVisibleItemPosition: Int, totalItemCount: Int) {
        if (visibleItemCount + lastVisibleItemPosition + VISIBLE_THRESHOLD >= totalItemCount) {
            ...
            if (immutableQuery != null) {
                repository.nextFilms(immutableQuery)
            }
        }
    }
